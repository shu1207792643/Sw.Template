...
namespace Sw.Template.Web.Helper
{
    public class InitAutofac
    {
        ...
        public static void InitAutofacs()//初始化方法
        {
            ...
            //_Builder.RegisterType<CollectView>().Named<UserControl>("数据采集");//注册方式二
            //_Builder.RegisterType<MeasureView>().Named<UserControl>("专测联调");//注册方式二
            //_Builder.RegisterType<UserManagement>().Named<UserControl>("用户管理");//注册方式二
            //_Builder.RegisterType<SystemSetting>().Named<UserControl>("系统配置");//注册方式二
            //_Builder.RegisterType<SystemTesting>().Named<UserControl>("系统自检");//注册方式二
        }
